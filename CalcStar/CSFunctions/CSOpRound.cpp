...
namespace CSTAR
{
	...
	int CSOpRound::OpEval(std::vector<CSRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful,int intCurrPos)
	{
		...
		try{
			...
			if(blnValid)
			{
				...
				dblOutput = CMATH::round(dblNumber,intNumDecimal);
