...
namespace CSTAR
{
	...
	//!Default Constructor
	CSOpTruncate::CSOpTruncate(): CSOpBase()
	{
		m_intNumInputs = 2;
	};
	//!Default Destructor
	CSOpTruncate::~CSOpTruncate(){};

	//!Perform the operation on the stack
	int CSOpTruncate::OpEval(std::vector<CSRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful,int intCurrPos)
	{
		...
		try{
			...
			if(blnValid)
			{
				...
				unsigned int intNumDecimal = int(CMATH::round(dblNumDecimal,0));
				dblOutput = CMATH::trunc(dblNumber,intNumDecimal);
