...
namespace CSTAR
{
	...
	//!Default Constructor
	CSOpCeiling::CSOpCeiling(): CSOpBase()
	{
		m_intNumInputs = 1;
	};
	//!Default Destructor
	CSOpCeiling::~CSOpCeiling(){};

	//!Perform the operation on the stack
	int CSOpCeiling::OpEval(std::vector<CSRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful,int intCurrPos)
	{
		//FUNCTION:ceil(input)
		...
		try{
			...
			if(blnValid)
			{
				...
				dblOutput = CMATH::ceil(dblInput1);
